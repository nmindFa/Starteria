...
export function Step2Page() {
  ...
  const [hipIAloading, setHipIALoading] = useState(false);
  ...
  const handleIaSuggestRiskHypotheses = () => {
    setHipIALoading(true);
    setTimeout(() => {
      const suggestions = [
        'Si no logramos que TI confirme ownership del proceso, el formulario será ignorado.',
        'Si el formulario no cubre casos especiales, el tiempo promedio no mejora en >50%.',
        'Si los responsables no registran timestamps, no podremos medir la métrica objetivo.'
      ];
      setTestCard(prev => ({ ...prev, hipotesisRiesgosa: prev.hipotesisRiesgosa.trim() ? prev.hipotesisRiesgosa : suggestions[0] }));
      setHipIALoading(false);
    }, 900);
  };
        ...
        <div className="max-w-2xl mx-auto">
          ...
          {activeModule === 'D' && (
            <div className="space-y-6">
              ...
              <div className="sticky top-3 z-20 border rounded-2xl p-4 bg-slate-50/95 backdrop-blur supports-[backdrop-filter]:bg-slate-50/80 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs text-slate-500" style={{ fontWeight: 700 }}>Resumen del experimento</p>
                  <p className="text-[11px] text-slate-400">Se arma automáticamente</p>
                </div>
                  ...
                  <CollapsibleSection title="Editar Test Card" open={testOpen} onToggle={() => setTestOpen(v => !v)}>
                    <div className="space-y-3">
                      <ActionFieldCard
                        ...
                        iaLabel={hipIAloading ? '✨ Generando…' : '✨ IA: sugerir 3 hipótesis riesgosas'}
                        onIA={handleIaSuggestRiskHypotheses}
